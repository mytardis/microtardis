...
# microtardis change start
    ...
    if not request.session['session_show_hidden']:
        ...
        c['datasets'] = Dataset.objects.exclude(experiment=experiment_id, pk__in=hidden_datasets)
        hidden_datafiles = Datafile_Hidden.objects.filter(hidden=True).values_list('datafile', flat=True)
        c['datafiles'] = Dataset_File.objects.exclude(dataset__experiment=experiment_id, pk__in=hidden_datafiles)
    else:
        # show all objects
...
# microtardis change start
    ...
    if not request.session['session_show_hidden']:
        ...
        for dataset in c['datasets']:
            datafiles[dataset] = Dataset_File.objects.exclude(dataset=dataset.id, pk__in=hidden_datafiles).count()
...
# microtardis change start
    ...
    if not request.session['session_show_hidden']:
        # hide hidden objects
        hidden_datafiles = Datafile_Hidden.objects.filter(hidden=True).values_list('datafile', flat=True)
        c['datafiles'] = Dataset_File.objects.exclude(dataset__pk=dataset_id, pk__in=hidden_datafiles)
    else:
        # show all objects
